...
class QAPollLogger
{
	 ...
	 * Returns true if the latest item submission by $userid is older than $mins minute.
	 */
	function isLatestItemSubmissionOlderThan($userid, $mins)
	{
		if($mins == 0)
			return true;

		$query = "SELECT NOW() - date > interval '" . $mins . " minute' FROM qapoll_log " .
			"WHERE userid='" . $userid . "' AND type='1' ORDER BY date DESC LIMIT 1";
		$log_object = db_result(it_query($query));
	...
	/**
	 * Temporary function, I can't seem to find a time diff function in PHP!
	 * Returns true if the latest item solution submission by $userid is older than $mins minute.
	 */
	function isLatestItemSolutionSubmissionOlderThan($userid, $mins)
	{
		if($mins == 0)
			return true;
