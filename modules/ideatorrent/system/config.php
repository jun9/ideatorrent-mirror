...
class QAPollConfig extends Model
{
	...
	function loadFromPost()
	{
		...
		$this->_data["interval_between_idea_submissions"] = $_POST['interval_between_idea_submissions'];
		$this->_data["interval_between_solution_submissions"] = $_POST['interval_between_solution_submissions'];

		//Now check all entries.
		if(View::isThemeNameValid($this->_data["selected_theme"]) == false)
			$errorMessage .= t("The selected theme is not valid.") . "<br />";
		if(!is_numeric($this->_data["default_number_item_per_page"]) || $this->_data["default_number_item_per_page"] < 1) 
			$errorMessage .= t("The number of ideas per page should be numeric and superior to 0.") . "<br />";
		if(!is_numeric($this->_data["choice_number_approvals_needed"]) || $this->_data["choice_number_approvals_needed"] < 0) 
			$errorMessage .= t("The number of ideas per page should be numeric and superior or equal to 0.") . "<br />";
		if(!is_numeric($this->_data["interval_between_idea_submissions"]) || $this->_data["interval_between_idea_submissions"] < 0) 
			$errorMessage .= t("The minimum interval between idea submissions should be numeric and superior or equal to 0.") . "<br />";
		if(!is_numeric($this->_data["interval_between_solution_submissions"]) || $this->_data["interval_between_solution_submissions"] < 0) 
			$errorMessage .= t("The minimum interval between solution submissions should be numeric and superior or equal to 0.") . "<br />";
	...
	function store()
	{
		...
		if(user_access($site->getData()->adminrole))
		{
			...
			QAWebsiteSite::getInstance()->setSetting("interval_between_idea_submissions", $this->_data["interval_between_idea_submissions"]);
			QAWebsiteSite::getInstance()->setSetting("interval_between_solution_submissions", $this->_data["interval_between_solution_submissions"]);
		}
