...
class ChoiceSolutionListModel extends ModelList
{
	 ...
	 * choice_solution_ids: A comma separated list of choice solution ids we want to restrict our search to.
	...
	function _loadData()
	{
		...
		$query = "SELECT " .
			...
			//Get the user extra infos
			(($this->_include_user_extra_data == true)?"users.name as username, ":"") . 

			//Get the choice<->solution link infos
			(($this->_include_solution_link_infos == true)?
			"qapoll_choice_solution_link.solution_number, qapoll_choice_solution_link.choiceid, qapoll_choice_solution_link.advertize, " .
			"qapoll_choice_solution_link.status, qapoll_choice_solution_link.selected, ":"") .

			"qapoll_choice_solution.* " .
	...
	function _buildChoiceSolutionListQuery_where()
	{
		...
		if($this->_states['deleted'] == false && $this->_include_solution_link_infos == true)
		{
			$where .= "AND qapoll_choice_solution.status != -2 ";
		}
		if($this->_states['new'] == false && $this->_include_solution_link_infos == true)
		{
			$where .= "AND qapoll_choice_solution.status != 1 ";
		}
		if($this->_states['duplicate'] == false && $this->_include_solution_link_infos == true)
		{
			$where .= "AND qapoll_choice_solution.status != -1 ";
		}

		if($this->_link_states['deleted'] == false && $this->_include_solution_link_infos == true)
		{
			$where .= "AND qapoll_choice_solution_link.status != -2 ";
		}
		if($this->_link_states['new'] == false && $this->_include_solution_link_infos == true)
		{
		...
		if($this->_choice_solution_ids != null)
		{
			$where .= "AND qapoll_choice_solution.id IN (" . $this->_choice_solution_ids . ") ";
		}
	...
	function _buildChoiceSolutionListQuery_orderby()
	{
		...
		{
			case "solutionumber":
				if($this->_include_solution_link_infos == true)
					$orderby = "qapoll_choice_solution_link.solution_number ";
				else
					$orderby = "qapoll_choice_solution.id ";
			break;
	...
	function setFilterParameters($getarray)
	{
		...
		if($getarray['choice_solution_ids'] != null)
		{
			//Check that all the given ids are indeed numeric.
			$array_ids = explode(", ", $getarray['choice_solution_ids']);
			$is_all_numeric = true;
			...
			if($is_all_numeric)
				$this->_choice_solution_ids = $getarray['choice_solution_ids'];
		}
	...
	function setDataFilter($filter_array)
	{
		...
		if($filter_array['include_minimal_data'] != null && $filter_array['include_minimal_data'] == true)
		{
			...
			$this->_include_solution_link_infos = false;
		}
		else
		{
			...
			if($filter_array['include_solution_link_infos'] != null && is_numeric($filter_array['include_solution_link_infos']))
				$this->_include_solution_link_infos = $filter_array['include_solution_link_infos'];
