...
class ChoiceModel extends Model
{
	...
	function store()
	{
		...
		if($this->_id == 0)
		{
			...
			if(QAPollLogger::getInstance()->isLatestItemSubmissionOlderThan($user->uid, 
				QAPollConfig::getInstance()->getValue("interval_between_idea_submissions")) == false)
			{
				$errorMessage .= t("Please wait !length minute(s) between submissions.",
							array(
								"!length" => QAPollConfig::getInstance()->getValue("interval_between_idea_submissions")
							)) . "<br />";
			}
			...
			if($errorMessage == "")
			{
				$nextid =  db_next_id("{qapoll_choice}_id");
				
				$query = "INSERT INTO qapoll_choice (id, pollid, title, specid, forumid, userid, date, description, categoryid, " .
