...
require_once "views/choicesolutionlist.json.php";
		...
		switch ($this->_page) {
			...
			case "contributor":
				...
				else if($this->_param1 == "delete_user_comments")
				{
					...
					//Block the account, only if uid != 1
					if($this_user->uid != 1)
						user_save($this_user, array('status' => 0));

					drupal_set_message(t("All the user comments have been deleted, and the account has been blocked."), 'notice_msg');
...
/**					if($GLOBALS['entrypoint']->getData()->filterArray['relation'] != null)
				...
				else if($this->_param2 == null)
				{
					...
					else if (array_key_exists('_solution_submitted',$_POST))
					{
						...
						if(QAPollLogger::getInstance()->isLatestItemSolutionSubmissionOlderThan($GLOBALS['user']->uid,
							QAPollConfig::getInstance()->getValue("interval_between_solution_submissions")) == false)
						{
							drupal_set_message(t("To prevent spam, there is a !length minute(s) minimum interval between each solution submission. Please wait.", 
								array(
									"!length" => QAPollConfig::getInstance()->getValue("interval_between_solution_submissions")
									)), 'error_msg');
							$view = new ChoiceView();
							$this->_page_title = strip_tags_and_evil_attributes($model->getData()->title);
							break;
	...
	function handleAjaxDataCalls()
	{
		...
		switch ($this->_page) {
			...
			//Find and return possible duplicates of a given title.
			case "ajaxdata_solution_votes":

				$model = new ChoiceSolutionListModel();
				$filter = array("choice_solution_ids" => $this->_param1);
				$model->setFilterParameters($filter);
				$model->setDataFilter(array("include_minimal_data" => true));

				$view = new ChoiceSolutionListJSONView();
				$template = "json_votes_only";
			break;
